...
val fsubst_def = Define`
  ...
  (fsubst v (fFORALL n ff) =
   let v' = (n =+ V n) v in
   let z = if (?y. y IN (ffvs (fFORALL n ff)) /\ n IN tfvs ((n =+ V n) v y))
           then VARIANT (ffvs (fsubst ((n =+ V n) v) ff)) else n in
	   fFORALL z (fsubst ((n =+ V z)v) ff)) /\

   (fsubst v (fEXISTS n ff) =
   let v' = (n =+ V n) v in
   let z = if (?y. y IN (ffvs (fEXISTS n ff)) /\ n IN tfvs ((n =+ V n) v y))
           then VARIANT (ffvs (fsubst ((n =+ V n) v) ff)) else n in
	   fEXISTS z (fsubst ((n =+ V z)v) ff))`
...
Proof
  ...
  >- (rw[EQ_IMP_THM, Once EXTENSION, PULL_EXISTS] >>
      rename [`y ∈ ffvs ff`, `a ∈ ffvs ff`, `x ∈ tfvs (v (|n |-> V n|) a)`] (* 2 *)
      >- (Cases_on `a = n` (* 2 *)
          >- (fs[APPLY_UPDATE_THM, tfvs_def] >> rw[] >> rfs[] >> metis_tac[])
          >- (fs[APPLY_UPDATE_THM, tfvs_def] >> rfs[] >> Cases_on `n = y` >> fs[APPLY_UPDATE_THM,tfvs_def,ffvs_def] >> metis_tac[]))
  ...
  >- (rw[EQ_IMP_THM, Once EXTENSION, PULL_EXISTS]
     >- (rename [`a ∈ ffvs ff`, `x ∈ tfvs (v (|n |-> V n|) a)`] >>
        Cases_on `a = n` (* 2 *)
	>- (fs[APPLY_UPDATE_THM, tfvs_def] >> rw[] >> rfs[]) 
	>- (fs[APPLY_UPDATE_THM, tfvs_def] >> rfs[] >> Cases_on `n = x'` >>
	   fs[tfvs_def,ffvs_def] >> metis_tac[]))
     >- (rename [`a ∈ ffvs ff`, `x ∈ tfvs (v a)`,`a <> n`] >>
...
Proof
  ...
  rw[EQ_IMP_THM] >> qexists_tac `x` >> rw[] >> first_x_assum (qspec_then `x` mp_tac) >> rw[] >>
  `feval M (interpret M σ ∘ v)⦇n ↦ x⦈ f = 


>> Cases_on `v y` >> fs[ffvs_def,tfvs_def] (* 2 *)
	      >- 




	   >- `n' <> VARIANT (ffvs (fsubst v⦇n ↦ V n⦈ f))` suffices_by fs[APPLY_UPDATE_THM] >>
	      `¬(n = y)` by fs[] >> fs[] >> simp[ffvs_fsubst] >>
	      `n' IN (BIGUNION (IMAGE (tfvs ∘ v⦇n ↦ V n⦈) (ffvs f)))` suffices_by cheat >>
	      fs[PULL_EXISTS] >>
