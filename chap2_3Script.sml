...
val SUBMODEL_satis_lemma = store_thm(
  "SUBMODEL_satis_lemma",
  ``!M1 M2. SUBMODEL M1 M2 ==>
            !w phi. w IN M2.frame.world /\ satis M2 w (DIAM phi) ==>
	           ? v. v IN 
 

val thm_2_34 = store_thm( 
  "thm_2_34",
  ``!M w:'b phi. INFINITE univ(:'b) /\  satis M w phi ==> ?FM v:'b. FINITE (FM.frame.world) /\ v IN FM.frame.world /\ satis FM v phi``,
  rw[] >>
  ...
  `w IN M3.frame.world`
   ...
   `FINITE ({f | DEG f ≤ k ∧ ∀a. VAR a ∈ subforms f ⇒ a ∈ s}//E μ)` by metis_tac[] >>
   `FINITE (IMAGE CHOICE ({f | DEG f ≤ k ∧ ∀a. VAR a ∈ subforms f ⇒ a ∈ s}//E μ))` by metis_tac[FINITE_BIJ,CHOICE_BIJ_partition,equiv0_equiv_on] >>
