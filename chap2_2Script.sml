...
bisim Z M M' <=> !w w'. w IN M.frame.world /\ w' IN M'.frame.world /\ Z w w' ==>
...
val bisim_world_def = Define`
bisim_world M M' w w' <=> ?Z. bisim Z M M' /\ w IN M.frame.world /\ w' IN M'.frame.world /\ Z w w'`
...
rpt strip_tac >- (`w' = f w` by metis_tac[] >> rw[] >> fs[satis_def,iso_def,strong_hom_def])
              >- (qexists_tac `f v` >> `w' = f w` by metis_tac[] >> rw[] >- fs[iso_def,strong_hom_def]
                                                                         >- metis_tac[iso_def,strong_hom_def])
              >- (fs[iso_def] >> `?x. x IN M.frame.world /\ f x = v'` by metis_tac[BIJ_DEF,SURJ_DEF] >>
...
val prop_2_19_iv = store_thm(
"prop_2_19_iv",
``!M M' f. bounded_mor_image f M M' ==> (!w. w IN M.frame.world ==> bisim_world M M' w (f w))``,
rw[bisim_world_def,bisim_def] >>
qexists_tac `λn1 n2. (n2 = f n1)` >>
rpt strip_tac >- (`w'' = f w'` by metis_tac[] >> rw[] >> fs[bounded_mor_image_def,bounded_mor_def])
              >- (`w'' = f w'` by metis_tac[] >> qexists_tac `f v` >> rw[] >>  metis_tac[bounded_mor_image_def,bounded_mor_def])
...
val thm_2_20_lemma = store_thm(
"thm_2_20_lemma",
``!M M' w w' form. bisim_world M M' w w' ==> (satis M w form <=> satis M' w' form)``,
Induct_on `form`
>- (rpt strip_tac
...
>- (rpt strip_tac
  ...
  >> metis_tac[satis_def])
>- (rpt strip_tac
  >> rw[satis_def] >> eq_tac >- (rpt strip_tac >- metis_tac[bisim_world_def]
                                               >- (`∃Z. bisim Z M M' ∧ w ∈ M.frame.world ∧ w' ∈ M'.frame.world ∧ Z w w'` by metis_tac[bisim_world_def] >> `∃v'. v' ∈ M'.frame.world ∧ Z v v' ∧ M'.frame.rel w' v'` by metis_tac[bisim_def,IN_DEF] >> `bisim_world M M' v v'` by metis_tac[bisim_world_def] >> qexists_tac `v'` >> metis_tac[IN_DEF]))
                             >- (rpt strip_tac >- metis_tac[bisim_world_def]
                                               >-
(`∃Z. bisim Z M M' ∧ w ∈ M.frame.world ∧ w' ∈ M'.frame.world ∧ Z w w'` by metis_tac[bisim_world_def]
>> `M'.frame.rel w' v` by metis_tac[IN_DEF]
>> `?a. a IN M.frame.world /\ Z a v /\ M.frame.rel w a` by metis_tac[bisim_def]
...
>> rpt strip_tac
>> `!form. satis M w form <=> satis M' w' form` suffices_by fs[SET_EQ_SUBSET,SUBSET_DEF] >> metis_tac[thm_2_20_lemma]);
...
>- (`?BIGCONJ. satis M w BIGCONJ <=> (!f. f IN s ==> satis M w f)` by metis_tac[]
>> qexists_tac `AND BIGCONJ e` >> eq_tac >- (rpt strip_tac >> `f = e \/ f IN s` by metis_tac[IN_INSERT]
                                                             >- (rw[] >> `satis M w e` by metis_tac[satis_def,AND_def]) >- (`satis M w BIGCONJ` by metis_tac[satis_def,AND_def] >> `∀f. f ∈ s ⇒ satis M w f` by metis_tac[] >> metis_tac
                                                             []))
                                          >- (rpt strip_tac
                                            ...
                                            >> rw[satis_def,AND_def]))
);
                                            


val image_finite_def = Define
`image_finite M <=> !x. x IN M.frame.world ==> FINITE {y| y IN M.frame.world /\ M.frame.rel x y}`;



val box_vac_true = store_thm(
"box_vac_true",
``!form w M. w IN M.frame.world /\ ¬(?w'. w' IN M.frame.world /\ M.frame.rel w w') ==> satis M w (BOX form)``,
rw[satis_def,BOX_def] >> metis_tac[IN_DEF]);



val diam_exist_true = store_thm(
"diam_exist_true",
``!M w. w IN M.frame.world /\ (?w'. w' IN M.frame.world /\ M.frame.rel w w') ==> satis M w (DIAM TRUE)``,
...
val FINITE_BIGCONJ = store_thm(
"FINITE_BIGCONJ",
``!s. FINITE s ==>
(s <> {} ==>
(v IN M.frame.world /\ (!v'. v' IN s ==> ?phi. satis M v phi /\ ¬satis M' v' phi)) ==>
...
>> `∀v'. (v' = e \/ v' IN s) ⇒ ∃phi. satis M v phi ∧ ¬satis M' v' phi` by fs[INSERT_DEF]
...
>> Cases_on `s <> {}`
  ...
  >- (strip_tac >> strip_tac >> `v' = e \/ v' IN s` by fs[INSERT_DEF] >- metis_tac[] >- metis_tac[NOT_IN_EMPTY]
  )
  )
)
);                       
...
>- (
 ...
 >> ho_match_mp_tac wlog_lemma >>
  rpt strip_tac >> simp[]
   ...
   >> `{u' | u' ∈ M'.frame.world ∧ M'.frame.rel n2 u'} <> {}` by metis_tac[MEMBER_NOT_EMPTY]
   >> `∀n2'.
   ...
   >> `∀n2'.
          n2' ∈ M'.frame.world /\ M'.frame.rel n2 n2' ==>
          (?form. satis M n1' form /\ ¬satis M' n2' form)` by metis_tac[satis_def]
   >> `∀n2'. n2' IN {u' | u' ∈ M'.frame.world ∧ M'.frame.rel n2 u'} ==>
       ∃form. satis M n1' form ∧ ¬satis M' n2' form` by simp[]
   >> `?psi. satis M n1' psi /\ (!n2'. n2' IN {u' | u' ∈ M'.frame.world ∧ M'.frame.rel n2 u'} ==> ¬satis M' n2' psi)` by metis_tac[FINITE_BIGCONJ]
   >> `satis M n1 (DIAM psi)` by (`n1 ∈ M.frame.world ∧
     ∃v. v ∈ M.frame.rel n1 ∧ v ∈ M.frame.world ∧ satis M v psi` suffices_by metis_tac[satis_def] >> metis_tac[IN_DEF])
