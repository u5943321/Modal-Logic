...
val Los_modal_thm = store_thm(
  ...
  ``!U J Ms. ultrafilter U J ==>
             !phi fc. (satis (ultraproduct_model U J Ms) fc phi <=>
	              (fc IN (ultraproduct_model U J Ms).frame.world /\
	              ?f. f IN fc /\ {i | i IN J /\ satis (Ms i) (f i) phi} IN U))``,
  strip_tac >> strip_tac >> strip_tac  >> strip_tac >> Induct_on `phi` >> rw[]
  >- (rw[satis_def,ultraproduct_world,ultraproduct_valt] >> eq_tac >> rw[]
     >> qexists_tac `f` >> rw[] >> fs[] >>
  ...
  >- (rw[satis_def,EQ_IMP_THM,ultraproduct_world] (* 7 *)
     >- metis_tac[]
     >- (qexists_tac `x` >> rw[])
     >- (qexists_tac `f` >> rw[] >> 
	...
	>> metis_tac[]))
  >- (rw[satis_def] >> metis_tac[EMPTY_NOTIN_ultrafilter])
  >- rw[satis_def,EQ_IMP_THM,ultraproduct_world]
     >- metis_tac[]
     >- metis_tac[]
     >- qexists_tac `f` >> rw[] >>
        `f NOTIN
         ...
         {i | i IN J /\ satis (Ms i) (f i) phi} NOTIN U` by metis_tac[]


fs[ultraproduct_world]
...
val prop_2_71 = store_thm(
  ...
  >- (rw[satis_def,EQ_IMP_THM] (* 4 *)
     ...
     >- (fs[ultraproduct_valt,Uequiv_def] >>
        `({i | i ∈ J ∧ w = f i} INTER {i | i ∈ J ∧ f i ∈ (MS i).valt a}) IN U`
	  by metis_tac[ultrafilter_def,proper_filter_def,filter_def] >>
	`{i | i ∈ J ∧ w = f i} ∩ {i | i ∈ J ∧ f i ∈ (MS i).valt a} <> {}` by metis_tac[EMPTY_NOTIN_ultrafilter] >>
	`?i. i IN {i | i ∈ J ∧ w = f i} ∩ {i | i ∈ J ∧ f i ∈ (MS i).valt a}` by metis_tac[MEMBER_NOT_EMPTY] >> fs[] >> metis_tac[])
     >- (simp[ultraproduct_world] >> rw[] (* 2 *)
        >- metis_tac[MEMBER_NOT_EMPTY]
	>- (qexists_tac `\i.w` >> rw[] >>
	   rw[Uequiv_def,models2worlds_def,Cart_prod_def] >> simp[EXTENSION] >> metis_tac[]))
     >- (simp[ultraproduct_valt] >> qexists_tac `\i.w` >> rw[] (* 2 *)
        >- (rw[Uequiv_def,Cart_prod_def,models2worlds_def] 
	   >- metis_tac[MEMBER_NOT_EMPTY]
	   >- metis_tac[ultrafilter_def,proper_filter_def,filter_def])
	>- (`{i | i ∈ J ∧ w ∈ (MS i).valt a} = J` suffices_by
	   metis_tac[ultrafilter_def,proper_filter_def,filter_def] >> simp[EXTENSION] >>
	   metis_tac[])))
  >- rw[satis_def,EQ_IMP_THM]
  >- rw[satis_def,EQ_IMP_THM]
  >- (rw[satis_def,EQ_IMP_THM] (* 2 *) >> metis_tac[ultraproduct_world_constant])
  >- rw[satis_def,EQ_IMP_THM]
     >- metis_tac[ultraproduct_world_constant]
     >- `(!i. i IN J ==> (MS i).frame.world <> {}) /\
         ?x.
            ...
            {y |
             (!i. i IN J ==> y i IN (MS i).frame.world) /\
             {i | i IN J /\ x i = y i} IN U}` by metis_tac[ultraproduct_world] >> fs[] >>
	Q.UNDISCH_THEN `{y |
       (!i. i IN J ==> y i IN (MS i).frame.world) /\
       {i | i IN J /\ x i = y i} IN U} IN
      (ultraproduct_model U J MS).frame.world` (K ALL_TAC) >>
      fs[ultraproduct_rel] >>
      Q.UNDISCH_THEN `v =
       {y |
        (!i. i IN J ==> y i IN (MS i).frame.world) /\
        {i | i IN J /\ x i = y i} IN U}` (K ALL_TAC) >>
      fs[Uequiv_def]
...
`?w'. v = {fw | Uequiv U J (models2worlds MS) (\i. w') fw}`
           by (simp[EXTENSION, Uequiv_def]  >>
               drule_then (fn th => fs[th]) ultraproduct_world >> rfs[] >>
	       rw[] >> simp[Cart_prod_def, models2worlds_def] >>
	       `!i. i IN J ==> ?x. x IN M.frame.world`
	         by metis_tac[MEMBER_NOT_EMPTY] >>
	       pop_assum (SUBST1_TAC o EQT_INTRO) >> simp[]


        `w' IN M.frame.world`
	   by ((* use ultraproduct_world_constant *) cheat) 
	`M.frame.rel w w'`
	   by (fs[ultraproduct_rel] >>
	       REV_FULL_SIMP_TAC (srw_ss() ++ boolSimps.CONJ_ss)[] >> cheat)
	metis_tac[])
