...
\begin{proof}
  ...
  %Suppose for a set $s$ of formulas, each element of it has only one free variable $x$ except the ones that are representing constants, and for each subset $ss\subseteq s$, we have a valuation that realises this subset in the ultraproduct model, with the `constants' delivered to the correct places, we will prove the whole set $s$ is realised in the ultraproduct model by a valuation which sends the `constants' to the right places. As the value of the constants are fixed, it amounts to find somewhere to send the only `real' free variable $x$ to. If $s$ is finite then there is nothing to prove, so we assume $s$ is infinite in the following discussion.

%Recall we have restricted ourself to countably first order language, then as proved in ??? the infinite set $s$ of first order formulas are countable. Hence we obtain a bijection \HOLinline{\HOLFreeVar{enum}} from the universe of natural numbers to our set $s$. Define \HOLinline{\HOLFreeVar{conj}} to be the primitive recursive function \HOLinline{\HOLConst{PRIM_REC}\;\HOLConst{True}\;(\HOLTokenLambda{}\HOLBoundVar{conjn}\;\HOLBoundVar{n}.\;\HOLConst{fAND}\;\HOLBoundVar{conjn}\;(\HOLFreeVar{enum}\;\HOLBoundVar{n}))}, then \HOLinline{\HOLFreeVar{conj}\;\HOLFreeVar{n}} will be the formula $enum(0)\land\cdots\land enum(n)$. Then it suffices to prove the existence of a point that realise any of these conjunctions. By \texttt{ultraproduct_suffices_rep} and the Los theorem, it amounts to find out an a function to represent this equivalence class, we need some setups to find out the function. 

%Consider the family of sets:
%
Type inference failure: unable to infer a type for the application of

(% :(α model -> (num -> α) -> form -> bool) -> β -> (γ -> δ) -> form -> bool)
...
%such $J_n$'s forms a desending chain. As by assumption, each finite subset of $s$ is realisable, each such $J$ is realisable, so all of them are in $U$ by Los theorem. By \texttt{countably_incomplete_chain}, we get another descending chain $I_n$'s of sets $U$ with $\bigcap_{n\in\mathbb N} I_n=\emptyset$. Define $X_n:= I_n\cap J_n$, then $X_n$ is a descending chain in $U$ where $\bigcap_{n\in\mathbb N}X_n=\emptyset$. Hence for any $i\in I$, it exists a number $N(i)$ such that $i\in X_{N(i)}$ and for any $a>N(i)$, $i\notin X_a$. The equivalence class we will send $x$ to is represented by the function 
%
Type inference failure: unable to infer a type for the application of

feval ((FMS :β -> α model) (i :β)) (% :num -> α)
